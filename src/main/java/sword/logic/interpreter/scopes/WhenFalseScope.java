 ...
 * that allow restricting the existing scope assuming that the expression will return false.
 */
public final class WhenFalseScope extends AbstractScope {
    private final AbstractScope mParent;
    private final Expression mCondition;

    WhenFalseScope(AbstractScope parent, Expression condition) {
    ...
    Type resolveType(Token constantName, Map<Expression, Type> resolvedExpressions, ImmutableMap<String, Type> restrictionMap) throws UnresolvedTypeReferenceException, UnresolvedReferenceException {
        if (resolvedExpressions.containsKey(mCondition)) {
            ...
            final EnumType alwaysFalseType = BuiltInScope.getInstance().getAlwaysFalseType();
            try {
                final ImmutableMap<String, Type> newRestrictionMap = mCondition.restrictionMap(alwaysFalseType, resolvedExpressions, restrictionMap);
